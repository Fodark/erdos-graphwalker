...
def add_coauthorship(tx, id_1, id_2, name, affiliation):
    query_string = """MATCH(a:Person {google_id: $id_1}) WITH a MERGE(a)-[:COAUTHOR]->(b:Person {google_id: $id_2, name: $name, affiliation: $affiliation})"""
    logging.debug("CONSUMER: adding relation between {} and {}".format(id_1, id_2))
    ...
    with driver.session() as session:
        for el in soup_profili_coauthors:
            ...
            if not r.get(id_coauthors):
                logging.debug("CONSUMER: enqueuing {}".format(id_coauthors))
...
while True:
    ...
    logging.debug("CONSUMER: analyzing {} w/ priority {}".format(id_, value))
    if not r.get(id_):
      time.sleep(.5)
